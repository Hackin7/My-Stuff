...
def conversion_func(row):
    ...
    ## Tagging
    entry["tags"] = {}
    for c in ["Category" ,"Impressiveness", "Platform", "Skills"]:
        entry["tags"][c] = readList(row[c], ', ')

    ## OTher Details
    entry["name"] = read(row["Name"])
    entry["description"] = read(row["Description"])
    entry["links"] = readList(row["Links"], '\n')
...
### Custom overall code ############################################################
def run():
    XLS_FILENAME = 'Achievements and Projects.xlsx'
    SHEET_NAME = 'Projects'
    OUT_FILENAME = '../../js/all_projects.js'
